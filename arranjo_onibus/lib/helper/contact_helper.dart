...
class ContactHelper {
    ...
    return await openDatabase(path, version: 1, onCreate: (Database db, int newerVersion) async{
      await db.execute(
          "CREATE TABLE $contactTable($idColumn INTEGER PRIMARY KEY, $nomeColumn TEXT,"
              "$rgColumn TEXT, $poltronaColumn TEXT, $grupoColumn TEXT, $diaColumn TEXT )");
    }
    );
  }
  ...
  Future<Contact> getContact(int id)async{
    ...
    columns: [idColumn, nomeColumn, rgColumn, poltronaColumn, grupoColumn, diaColumn],
    where: "$idColumn =?",
    whereArgs: [id]);
    if(maps.length > 0){
...
class Contact{
  ...
  @override
  String toString() {
    return "Contact(id: $id, nome: $nome, rg: $rg, poltrona:$poltrona, grupo: $grupo, dia: $dia )";
